<script>
  ...
  import { state, projects } from "../../storesold";
...
<div in:fade={{ duration: 100 }}>
  <Field label={$_('export.template')}>
    <ButtonGroup>
      <Button on:click={() => (selectTemplate = true)} block={true}>
        {#if form.template !== ''}
          <span class="lnr lnr-checkmark-circle" />
          {form.template.name}
  ...
  <ButtonGroup>
    <Button on:click={download} disabled={!checkForm} block={true}>
